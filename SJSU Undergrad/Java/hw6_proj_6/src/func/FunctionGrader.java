...
public class FunctionGrader {
	...
	private static void checkComplex() {
		...
		for (Constructor c : complexClass.getConstructors()) {
			if (c.getGenericParameterTypes().length == 2 && c.getGenericParameterTypes()[0].toString().equals("double")
		...
		for (int r = -100; r <= 100; r++) {
			for (int i = -100; i <= 100; i++) {
				...
				try {
					...
					if (!closeEnough(r, dr)) {
						String err = "c = new Complex(" + r + ", " + i + "; c.getReal() + returned " + dr
								+ ", expected " + r;
						failComplex(err);
						return;
					}
					...
					if (!closeEnough(di, i)) {
						String err = "c = new Complex(" + r + ", " + i + "; c.getImaginary() + returned " + di
								+ ", expected " + i;
						failComplex(err);
						return;
					}
			...
			for (int i = -100; i <= 100; i++) {
				...
				try {
					...
					if (!closeEnough(retrievedR, r)) {
						failComplex("c1 = new Complex(" + r + ", " + i
								+ "); c2 = new Complex(c1); c2.getReal() returned " + retrievedR + ", should be " + r);
						return;
					}
					double retrievedI = (double) getImaginaryMethod.invoke(c2);
					if (!closeEnough(retrievedI, i)) {
						failComplex("c1 = new Complex(" + r + ", " + i
								+ "); c2 = new Complex(c1); c2.getImaginary() returned " + retrievedI + ", should be "
								+ i);
						assert false;
						return;
					}
				...
				for (int r2 = -10; r2 <= 10; r2++) {
					for (int i2 = -10; i2 <= 10; i2++) {
						try {
							...
							GoldenComplex golden = GoldenComplex.multiply(new GoldenComplex(r1, i1),
									new GoldenComplex(r2, i2));
							if (Math.abs(prodR - golden.real) > 1.0e-6 || Math.abs(prodI - golden.imaginary) > 1.0e-6) {
								String err = "c1 = new Complex(" + r1 + ", " + i1 + ");\n";
								err += "c2 = new Complex(" + r2 + ", " + i2 + ");\n";
								err += "Complex.multiply(c1, c2) returned " + new GoldenComplex(prodR, prodI)
										+ ", should be " + golden;
								failComplex(err);
								return;
							}
	...
	private static void checkSubtract() {
		...
		try {
			...
			if (!implementsTheInterface(clazz)) {
				sop("SubtractionFunction class doesn't declare that it implements the interface, deducting "
						+ POINTS_PER_FUNCTION);
		...
		} // 5 obscure exceptions that should never get thrown
	...
	private static void checkMod() {
		...
		try {
			...
			if (!implementsTheInterface(clazz)) {
				sop("ModFunction class doesn't declare that it implements the interface, deducting "
						+ POINTS_PER_FUNCTION);
				points -= POINTS_PER_FUNCTION;
				return;
			}
	...
	private static void checkHypot() {
		...
		try {
			...
			if (!implementsTheInterface(clazz)) {
				sop("HypotFunction class doesn't declare that it implements the interface, deducting "
						+ POINTS_PER_FUNCTION);
				points -= POINTS_PER_FUNCTION;
				return;
			}
	...
	private static void checkComplexNorm() {
		...
		try {
			...
			if (!implementsTheInterface(clazz)) {
				sop("ComplexNormFunction class doesn't declare that it implements the interface, deducting "
						+ POINTS_PER_FUNCTION);
				points -= POINTS_PER_FUNCTION;
				return;
			}
	...
	private static void checkComplexSquared() {
		...
		try {
			...
			if (!implementsTheInterface(clazz)) {
				sop("ComplexSquaredNormFunction class doesn't declare that it implements the interface, deducting "
						+ POINTS_PER_FUNCTION);
				points -= POINTS_PER_FUNCTION;
				return;
			}
